...
function SpeakButton({ text }: { text: string }) {
  ...
  function speak() {
    if (!("speechSynthesis" in window)) { alert("المتصفح مش بيدعم TTS."); return; }
  ...
  return (
    <button
      type="button"
      onClick={speak}
      className="mt-2 flex items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-muted hover:text-primary transition-colors"
      title={playing ? "إيقاف" : "استمع"}
    >
      {playing ? <Pause className="size-3.5 text-primary" /> : <Play className="size-3.5" />}
      <Volume2 className="size-3.5" />
      <span>{playing ? `${fmt(elapsed)} / ${duration ?? fmt(estimatedSecs)}` : fmt(estimatedSecs)}</span>
    </button>
  );
}
