  ...
  return (
    <div>
      <div className="text-center">
        <h1 className="font-display text-3xl font-semibold text-white">{isSignup ? t("auth.createAccount") : t("auth.welcomeBack")}</h1>
      ...
      <div className="my-6 flex items-center gap-3">
        ...
        <span className="font-mono text-[10px] uppercase tracking-[0.12em] text-muted">{t("auth.or")}</span>
        <div className="h-px flex-1 bg-white/10" />
      </div>
      ...
      <form className="space-y-4" onSubmit={onSubmit} noValidate>
        {isSignup ? (
          <>
            <FormField label={t("auth.name")} error={"name" in errors ? errors.name?.message : undefined} id="name">
            ...
            <FormField label={t("auth.company")} error={"company" in errors ? errors.company?.message : undefined} id="company">
        ...
        <FormField label={t("auth.email")} error={errors.email?.message} id="email">
        ...
        <FormField label={t("auth.password")} error={errors.password?.message} id="password">
        ...
        <Button className="w-full" type="submit" disabled={form.formState.isSubmitting}>
          ...
          {isSignup ? t("auth.createWorkspace") : t("auth.signIn")}
          <ArrowRight className="size-4" />
        </Button>
      </form>

      <p className="mt-7 text-center text-sm text-muted">
        {isSignup ? t("auth.alreadyAccess") : t("auth.noAccount")}{" "}
