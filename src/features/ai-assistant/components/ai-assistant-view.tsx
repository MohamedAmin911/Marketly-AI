...
import { Image, Loader2, Mic, Paperclip, PauseCircle, Send, Share2, Sparkles, Square, Volume2, X } from "lucide-react";
...
const promptActions = [
  {
    ...
    icon: Sparkles,
    prompt:
      "Based on the current campaign analytics and brand context, draft a concise marketing proposal. Include key objectives, recommended channels, estimated budget allocation, and expected outcomes.",
  },
  {
    label: "Share meta metrics",
    icon: Share2,
    prompt:
      "Summarize the latest Meta (Facebook/Instagram) campaign metrics. Include impressions, CTR, conversions, ROI, and your top 3 recommendations to improve performance.",
  },
  {
    label: "Pause LinkedIn campaign",
    icon: PauseCircle,
    prompt:
      "Analyze the current LinkedIn campaign performance. Should I pause it? Give me a clear yes/no recommendation with supporting data and suggest what to do with the freed budget.",
  },
];
...
export function AiAssistantView() {
  const { attachment, draft, isSending, messages, sendMessage, setAttachment, setDraft, voiceEnabled, setVoiceEnabled } = useAssistantChat();
...
recognition.onresult = (event: any) => {
  ...
  async function handleFileSelected(file: File | undefined) {
    ...
    // For text/csv files, also read the text content so RAG can use it
    ...
    if (file.type === "text/plain" || file.type === "text/csv") {
      ...
      try {
        ...
        textContent = `[PDF file: ${file.name} — could not extract text]`;
      }
    }
        ...
        <StaggeredList className="space-y-5" aria-live="polite">
          {messages.map((message) => (
            <StaggeredItem key={message.id}>
              <ChatMessage
                message={message}
                onSpeak={voiceEnabled && message.role === "assistant" && message.id === lastMessageId ? playTTS : undefined}
              />
            </StaggeredItem>
          ))}
        </StaggeredList>
        ...
        <Card className="p-3">
          ...
          <div className="flex items-center justify-between border-t border-white/10 pt-3">
            <div className="flex items-center gap-1">
              ...
              {/* Voice input */}
              <Button variant="ghost" size="icon" type="button" aria-label={isListening ? "Stop recording" : "Voice input"} onClick={() => void toggleVoice()} className={isListening ? "text-red-400 animate-pulse" : ""}>
